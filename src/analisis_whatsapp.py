...
def calcula_usuarios(log: List[Mensaje]) -> List[str]:
    '''
    Devuelve una lista ordenada con los usuarios que aparecen en el log,
      sin duplicados.
    ...
    '''
    return sorted(
        {mensaje.usuario for mensaje in log}
    )
...
def cuenta_mensajes_por_usuario(log: List[Mensaje]) -> Dict[str, int]:
    ...
    :rtype: Dict[str, int]
    '''
    return Counter(m.usuario for m in log)
...
def muestra_numero_mensajes_por_usuario(log: List[Mensaje]) -> None:
    ...
    num_mensajes_por_usuario = cuenta_mensajes_por_usuario(log)
    usuarios = [] 
    num_mensajes = []
    for usuario, conteo in num_mensajes_por_usuario.items():
        usuarios.append(usuario)
        num_mensajes.append(conteo)
    
    # El código anterior hace lo contrario de zip
    # Hay una forma de usar zip para pedirle que haga eso:
    # A partir de una lista de tuplas, obtener n listas con cada una 
    # de las posiciones de las tuplas por separado ("unzip")
    lista_items = list(num_mensajes_por_usuario.items())
    usuarios, num_mensajes = zip(*lista_items)
...
def cuenta_mensajes_por_meses(log: List[Mensaje]) -> Dict[str, int]:
    '''
    Devuelve un diccionario en el que las claves son los meses a lo largo
      de los años 
    (por ejemplo, "2/2016", "3/2016",...) y los valores son el número
      de mensajes de cada mes/año.
    ...
    :rtype: Dict[str, int]
    '''
    return Counter(m.fecha.strftime("%m/%Y") for m in log)
...
def cuenta_mensajes_por_dia_semana(log: List[Mensaje]) -> Dict[str, int]:
    ...
    '''
    dias = ["L", "M", "X", "J", "V", "S", "D"] 
    return Counter(dias[m.fecha.weekday()] for m in log)
...
def cuenta_mensajes_por_momento_del_dia(log: List[Mensaje]) -> Dict[str, int]:
    ...
    :rtype: Dict[str, int]
    '''
    return Counter(momento_dia(m.hora.hour) for m in log)
...
def calcula_media_horas_entre_mensajes(log: List[Mensaje]) -> float:
    ...
    :rtype: float
    '''
    # Calcular las horas entre mensajes consecutivos
    horas_entre_mensajes = []
    for m1, m2 in zip(log, log[1:]):
        fechahora1 = datetime.combine(m1.fecha, m1.hora)
        fechahora2 = datetime.combine(m2.fecha, m2.hora)
        horas = (fechahora2 - fechahora1).total_seconds()/3600
        horas_entre_mensajes.append(horas)
    
    return sum(horas_entre_mensajes)/len(horas_entre_mensajes)
